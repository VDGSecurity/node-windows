...
var killkid = function(c){
  forcekill = true;
  if (child && argv.s) {
    return child.send('shutdown');
  } else if(child) {
    return child.kill();
  }
  // Consider this the forcekill state.
  log.error(argv.f+' exiting with error code '+c+', no child process could be stopped');
  process.exit();
}
